...
export default {
	async fetch(request: Request, env: any): Promise<Response> {
		...
		if (!isProdMode && url.pathname === '/') {
			return new Response('Hello YT world!');
		}
		//#endregion
		...
		if (url.pathname === '/youtube-playlist-videos') {
			...
			try {
				...
				let existingPlaylist: YtPlaylistVideo[] = [];
				try {
					const data = await env.KV_DATABASE_ONLINE_NAME.get(playlistId);
					existingPlaylist = (data ? JSON.parse(data) : []) as YtPlaylistVideo[];
				} catch (error) {}

				let videos: YtPlaylistVideo[] = [];
				if (isProdMode) {
					videos = existingPlaylist;
				} else {
					try {
						videos = await getYoutubePlaylistVideos(playlistId);
						...
						if (!isEqual(newIds, existedIds)) {
							await env.KV_DATABASE_ONLINE_NAME.put(playlistId, JSON.stringify(videos));
						}
					} catch (error) {
						videos = existingPlaylist;
					}
